...
usertrap(void)
{
    ...
    //Diavazontas auto to pdf https://pdos.csail.mit.edu/6.828/2019/lec/l-usingvm.pdf pou grafei ta exceptions me ton arithmo tous
    //Leei oti to scause me arithmo 13 einai PAGE FAULT pou proklithike apo read
    ...
    //Ara auta ta dyo mas endiaferoun
        ...
        if (!(error_page >= MAXVA))
        {
            ...
            if (addr && (*addr & PTE_COW)) {
                //new page and old page
                uint64 n_page, o_page, err;

                //Kanoume to COW flag: 0  kai to WRITE flag 1 giati tha ftiaksoume kainouria selida idia me authn opote prepei na exei kanonika flags
                int flgs = PTE_W | (PTE_FLAGS(*addr) ^ PTE_COW) ;
                ...
                err = mappages(p->pagetable, error_page, PGSIZE, n_page, flgs);
                if (err == -1)
